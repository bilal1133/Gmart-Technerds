...
export default class Coupon extends Component<AppProps, AppState> {
    ...
    async removeCoupon(id: number) {
        //remove coupons by id
        const {data: data} = await axios.post('/api/coupons/remove', {id});
        if (data)
            await $('#dataTable').DataTable().ajax.reload();
    }

    async removeCouponByIds(ids: any) {
        //remove lang by id
        const {data: data} = await axios.post('/api/coupons/delete', {ids: ids});
        if (data) {
    ...
    render() {
        if (this.state.redirectToEdit)
            return (<Redirect to={`/coupon/edit/${this.state.id}`}/>);

        return (
            <Layout>
                <Head>
                    <title>Gmarket - Coupon list</title>
                    <meta name="viewport" content="initial-scale=1.0, width=device-width"/>
                </Head>
                <h1 className="h3 mb-2 text-gray-800">Coupon</h1>
                <p className="mb-4">Create, remove and edit coupon</p>
                <div className="card shadow mb-4">
                    <div className="row card-header py-3">
                        <div className="col col-md-6 col-sm-6">
                            <h6 className="m-0 font-weight-bold text-primary">Coupon</h6>
                        </div>
                        <div className="col col-md-6 col-sm-6">
                            {
                                roleGuard('coupon.add') && (

                                    <Link to="/coupon/add">
                                        <button className="btn btn-success float-right">Add Coupon</button>
                                    </Link>

                                )
                            }
                            {
                                roleGuard('coupon.delete') && this.state.checkboxChecked && (
                                    <button className="btn btn-danger float-right mr-1" onClick={() => {
                                        this.removeCouponByIds(this.state.checkIds);
                ...
                <DeleteModal onSubmit={() => this.removeCoupon(this.state.id)}/>
                {
                    this.state.loading && (
                        <SmallLoader/>
                    )
