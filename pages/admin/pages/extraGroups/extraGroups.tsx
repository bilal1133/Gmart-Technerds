...
export default class ExtraGroups extends Component<AppProps, AppState> {
    ...
    async removeExtraGroup(id: number) {
        //remove extra group by id
        const {data: data} = await axios.post('/api/extraGroups/remove', {id});
        if (data)
            await $('#dataTable').DataTable().ajax.reload();
    }

    async removeExtraGroupsByIds(ids: any) {
        //remove lang by id
        const {data: data} = await axios.post('/api/extraGroups/delete', {ids: ids});
        if (data) {
    ...
    render() {
        if (this.state.redirectToEdit)
            return (<Redirect to={`/products/extraGroups/edit/${this.state.id}`}/>);

        return (
            <Layout>
                <Head>
                    <title>Gmarket - Extra groups list</title>
                    <meta name="viewport" content="initial-scale=1.0, width=device-width"/>
                </Head>
                <h1 className="h3 mb-2 text-gray-800">Extra groups</h1>
                <p className="mb-4">Create, remove and edit extra groups</p>
                <div className="card shadow mb-4">
                    <div className="row card-header py-3">
                        <div className="col col-md-6 col-sm-6">
                            <h6 className="m-0 font-weight-bold text-primary">Extra groups</h6>
                        </div>
                        <div className="col col-md-6 col-sm-6">
                            {
                                roleGuard('extraGroup.add') && (

                                    <Link to="/products/extraGroups/add">
                                        <button className="btn btn-success float-right">Add Extra group</button>
                                    </Link>

                                )
                            }
                            {
                                roleGuard('extraGroup.delete') && this.state.checkboxChecked && (
                                    <button className="btn btn-danger float-right mr-1" onClick={() => {
                                        this.removeExtraGroupsByIds(this.state.checkIds);
                                    }}>Delete Extra groups</button>
                                )
                            }
                        </div>
                    </div>
                ...
                <DeleteModal onSubmit={() => this.removeExtraGroup(this.state.id)}/>
                {
                    this.state.loading && (
                        <SmallLoader/>
                    )
