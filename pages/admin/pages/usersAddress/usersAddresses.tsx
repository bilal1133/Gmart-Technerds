...
export default class UsersAddresses extends Component<AppProps, AppState> {
    ...
    componentDidMount() {
        setTimeout(() => {
            ...
            $('#dataTable').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "/api/users/address/datatable",
                    "dataType": "json",
                    "type": "POST",
                    "data": {}
                },
    ...
    async removeUserAddress(id: number) {
        //remove user by id
        const {data: data} = await axios.post('/api/users/address/remove', {id});
        if (data) {
    ...
    async removeUserAddressesByIds(ids: any) {
        //remove lang by id
        const {data: data} = await axios.post('/api/users/address/delete', {ids: ids});
        if (data) {
            $("#checkboxAll").prop("checked", false);
            $('#dataTable').DataTable().ajax.reload();
            this.setState({
    ...
    render() {
        if (this.state.redirectToEdit)
            return (<Redirect to={`/users/address/edit/${this.state.id}`}/>);

        return (
            <Layout>
                <Head>
                    <title>Gmarket - Client addresses list</title>
                    <meta name="viewport" content="initial-scale=1.0, width=device-width"/>
                </Head>
                <h1 className="h3 mb-2 text-gray-800">Client addresses</h1>
                <div className="card shadow mb-4">
                    <div className="row card-header py-3">
                        <div className="col col-md-6 col-sm-6">
                            <h6 className="m-0 font-weight-bold text-primary">Client addresses</h6>
                        </div>
                        <div className="col col-md-6 col-sm-6">
                            {
                                roleGuard('users.user.add') && (

                                    <Link to="/users/addresses/add">
                                        <button className="btn btn-success float-right">Add Client Address</button>
                                    </Link>

                                )
                            }
                            {
                                roleGuard('categories.add') && this.state.checkboxChecked && (
                                    <button className="btn btn-danger float-right mr-1" onClick={() => {
                                        this.removeUserAddressesByIds(this.state.checkIds);
                ...
                <DeleteModal onSubmit={() => this.removeUserAddress(this.state.id)}/>
            </Layout>
        );
    }
}